...
describe('Test API to get a particular customer', () => {
  ...
  test('Should get 404 when wrong customer id provided', async () => {
    ...
    const response = await request(app)
      .get('/customers/' + customerId.toHexString())
      .set('Authorization', `Bearer ${token}`)
    expect(response.status).toBe(404)
    expect(response.body.errors).toBeInstanceOf(Array)
    expect(response.body.errors.length).toBe(1)
  ...
  test('Should get 200 when customer found', async () => {
    ...
    const response = await request(app)
      .get(`/customers/${result.insertedId.toHexString()}`)
      .set('Authorization', `Bearer ${token}`)
    ...
    expect(response.body.name).toBe('XYZ')
    expect(response.body.email).toBe('xyz@gmail.com')
